...
document.onmousedown = () => (mouseCondition = true);
document.onmouseup = () => (mouseCondition = false);
document.ontouchend = () => (lastTouchedBox = null);
const buttonClickedAndHovered = (e) => {
  ...
  switch (colour.textContent) {
    case 'Black':
      if (
        parseFloat(rgbValues[0]) === 0.0 &&
        parseFloat(rgbValues[1]) === 0.0 &&
        parseFloat(rgbValues[2]) === 0.0
      ) {
        break;
      } else {
        for (let i = 0; i < 3; i++) {
...
function createGrid(Size) {
  ...
  for (let i = 0; i < Size * Size; i++) {
    ...
    box.addEventListener('mouseover', (e) => {
      if (mouseCondition) buttonClickedAndHovered(e);
    });
    box.addEventListener('touchmove', (e) => {
      const touch = e.touches[0];
      const element = document.elementFromPoint(touch.clientX, touch.clientY);
      if (element && element.classList.contains('grid-container')) {
        if (element !== lastTouchedBox) {
          lastTouchedBox = element;
          buttonClickedAndHovered({ target: element });
        }
      }
      console.log(touch);
    });
